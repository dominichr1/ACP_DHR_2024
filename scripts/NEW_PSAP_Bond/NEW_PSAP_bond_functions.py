...
def add_filters(df):
    ...
    df['FChanged_diff'] = df['FChanged'].diff() 
    df.loc[df['deltaT_forward'] >= 10, 'FChanged_diff'] = 1 #more than 5 minutes difference assume a change of filter
    df.loc[df['deltaT_behind'] >= 10, 'FChanged_diff'] = 1 #more than 5 minutes

    df.loc[df['diff_I'] > 0.05, 'FChanged_diff'] = 1   #difference greater than 0.1
    df.loc[df['diff_Io'] > 0.05, 'FChanged_diff'] = 1   #difference greater than 0.1
    df.loc[df['Lnr'] < -0.1, 'FChanged_diff'] = 1
    df.loc[df['FChanged_diff'] == 1,'FChanged_diff'] = 1

    df['Filter_cum'] = (df['FChanged_diff'] == 1).cumsum()
    print("Maximum number of filters: "+str(df['Filter_cum'].max()))
    return df
